...
import { classifyComparisonModality } from "@/lib/compliance/compare";
...
const supportedUnits = new Set(["A", "kA", "V", "kV", "W", "kW", "MW", "Hz", "bar", "kPa", "MPa", "Pa", "psi", "°C", "°F", "C", "F", "%", "l/s", "L/s", "LPM", "GPM", "m3/h", "m³/h", "rpm", "mm", "cm", "m", "ms", "s", "min", "h", "day", "days"]);

function numericFromText(text: string) {
  const match = text.match(/(-?\d+(?:\.\d+)?)\s*(MPa|kPa|Pa|bar|psi|kA|A|kV|V|MW|kW|W|Hz|°C|°F|C|F|%|l\/s|L\/s|LPM|GPM|m3\/h|m³\/h|rpm|mm|cm|m|ms|s|min|h|days?)(?:\s*[±+]\s*(\d+(?:\.\d+)?))?/i);
