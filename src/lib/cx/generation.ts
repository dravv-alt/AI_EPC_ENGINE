...
export async function generateChecklistDraft(checklistId: string, actorId: string) {
  ...
  try {
    ...
    if (versionIds.length) {
      ...
      const semanticResults = await retrieveSemanticCitations({ projectId: checklist.projectId, query: searchQuery, limit: 20 });
      // Filter to only regions from selected standard versions
      const semanticRegionIds = new Set(semanticResults.map((result) => result.sourceRegionId));
