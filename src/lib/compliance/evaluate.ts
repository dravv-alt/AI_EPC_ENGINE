...
export function evaluateComplianceGoldenSet(input: unknown) {
  ...
  const rows = dataset.cases.map((testCase) => {
    ...
    return {
      ...
      passed: result.verdict === testCase.expectedVerdict,
      reason: result.reason,
    };
  });
  const positive = (verdict: string) => verdict === "deterministic_flag";
  const truePositive = rows.filter(
    (row) => positive(row.expected) && positive(row.actual),
  ).length;
  ...
  const trueNegative =
    rows.length - truePositive - falsePositive - falseNegative;
  ...
  const byModality = Object.fromEntries(
    ["numeric", "boolean", "categorical", "narrative"].map((modality) => {
