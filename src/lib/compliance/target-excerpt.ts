const stopWords = new Set([
  ...
  "of", "on", "or", "shall", "should", "that", "the", "this", "to", "with",
]);
 ...
 */
export function selectControlledTargetExcerpt(requirement: string, targetRegionText: string) {
  ...
  const requirementNumbers = new Set(requirementTokens.filter((token) => /^\d/.test(token)));

  const scored = fragments.map((fragment, index) => {
    const fragmentTokens = tokens(fragment);
    const fragmentSet = new Set(fragmentTokens);
    const overlap = [...requirementSet].filter((token) => fragmentSet.has(token)).length;
    const nonNumericOverlap = [...requirementSet].filter((token) => !/^\d/.test(token) && fragmentSet.has(token)).length;
    const numericOverlap = [...requirementNumbers].filter((token) => fragmentSet.has(token)).length;
    const measurementSignal = /\d\s*(?:kpa|pa|bar|psi|kv|v|mw|kw|w|hz|°c|c|%|l\/s|lpm|m3\/h|m³\/h|rpm|mm|cm|m)\b/i.test(fragment) ? 1 : 0;
    const controlledSignal = /\b(?:shall|must|required|rating|type|class|material|provided|available|value|capacity|pressure|temperature|flow|voltage)\b/i.test(fragment) ? 1 : 0;
    return { fragment, index, score: nonNumericOverlap * 5 + overlap * 2 + numericOverlap * 2 + measurementSignal + controlledSignal };
