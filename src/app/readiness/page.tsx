...
export default async function ReadinessPage({ searchParams }: { searchParams: Promise<{ gate?: string; id?: string }> }) {
  const projectId = await getActiveProjectId();
  const { gate, id } = await searchParams;
  // `id` keeps legacy links from the discarded Command Center workbench useful;
  ...
  return <FeatureShell projectName={data.project} eyebrow="Deterministic rules" title="Readiness & decisions" description="Inspect every proof category, prerequisite and blocker behind a gate state. Schedule status is cross-linked but never changes readiness.">
    <div className="workflow-stack">
      ...
      return <article id={`gate-${context.gate.id}`} className={`surface readiness-detail-card ${context.gate.id === focusedGateId ? "is-selected" : ""}`} key={context.gate.id}>
        ...
        {!readiness || readiness.acceptedRequirements === 0 ? <section className="readiness-guidance"><h3>UNKNOWN — configuration incomplete</h3><p>Map at least one accepted, cited requirement to this gate. No evidence or empty state is treated as success.</p><Link className="button button-secondary" href="/graph">Map requirements</Link></section> : <div className="proof-grid">{(["accepted", "missing", "unapproved", "stale", "failed"] as const).map((state) => { const stateProofs = readiness.proofDetails.filter((item, index, items) => item.state === state && items.findIndex((candidate) => candidate.requirementId === item.requirementId) === index); return <section className={`proof-column proof-${state}`} key={state}><h3>{labels[state]} <span>{stateProofs.length}</span></h3>{stateProofs.map((item) => <article key={`${state}:${item.requirementId}`}><b>{item.statement}</b><small>{item.evidenceIds.length ? `${item.evidenceIds.length} linked evidence record(s)` : "No linked evidence"}</small></article>)}</section>; })}</div>}
        <div className="readiness-secondary-grid"><section><h3>Blocking findings ({readiness?.blockingFindingDetails.length ?? 0})</h3>{readiness?.blockingFindingDetails.map((finding) => <Link className="entity-row" href={`/actions?finding=${finding.id}`} key={finding.id}><div>{finding.isOverdue && <span className="status-pill blocked">overdue</span>}<b>{finding.title}</b><span>{finding.severity} · {finding.dueAt ? `due ${new Intl.DateTimeFormat("en-IN", { dateStyle: "medium" }).format(finding.dueAt)}` : "unscheduled"} · {finding.reasons.includes("severity") && finding.reasons.includes("overdue") ? "severity + overdue" : finding.reasons.includes("overdue") ? "overdue, any severity" : "severity"}</span></div></Link>)}{!readiness?.blockingFindingDetails.length && <p className="workflow-hint">No open high/critical or overdue finding blocks this gate.</p>}</section><section><h3>Prerequisites ({readiness?.unmetPrerequisites ?? 0})</h3>{readiness?.prerequisiteDetails.map((gate) => <Link className="entity-row" href={`/readiness?gate=${gate.id}`} key={gate.id}><div><b>{gate.name}</b><span>{gate.status.replaceAll("_", " ")}</span></div></Link>)}{!readiness?.unmetPrerequisites && <p className="workflow-hint">All preceding gates are approved or this is the first gate.</p>}</section><section><h3>Schedule status — informational only</h3>{context.schedule.tasks.map((task) => <Link className="entity-row" href={`/schedule?task=${task.id}`} key={task.id}><div><b>{task.name}</b><span>{task.assignment ? `${new Intl.DateTimeFormat("en-IN", { dateStyle: "medium" }).format(task.assignment.startAt)} → ${new Intl.DateTimeFormat("en-IN", { dateStyle: "medium" }).format(task.assignment.endAt)}` : "Not assigned in current version"}</span></div><small>{task.assignment?.isCritical ? "Critical path" : "Linked task"}</small></Link>)}{!context.schedule.tasks.length && <p className="workflow-hint">No schedule tasks are linked to this gate.</p>}</section></div>
