...
export async function POST(request: Request, { params }: { params: Promise<{ evidenceId: string }> }) {
  ...
  try {
    ...
    await db.transaction(async (tx) => { await tx.update(evidence).set({ validityState: next, updatedAt: new Date() }).where(eq(evidence.id, evidenceId)); if (next === "accepted" && linked.length) await tx.insert(edges).values(linked.map((requirement) => ({ projectId: record.projectId, fromType: "evidence", fromId: record.id, relationshipType: "PROVES", toType: "requirement", toId: requirement.id }))); });
