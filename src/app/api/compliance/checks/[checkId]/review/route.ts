...
import {
  deriveFindingOwner,
  findingDueAt,
} from "@/lib/compliance/create-check";
...
export async function GET(
  ...
  const existing = await db.query.complianceChecks.findFirst({
  ...
  try {
    ...
    const advisory = await surfaceTeachbackAdvisory({
      projectId: existing.projectId,
      subjectType: "compliance_check",
      queryText: existing.reason,
    });
    ...
    return NextResponse.json(
      {
        error:
          error instanceof Error
            ? error.message
            : "Unable to load advisory context",
      },
...
const schema = z
  .object({
    ...
    finalVerdict: z
      .enum(["conforms", "deterministic_flag", "possible_mismatch"])
      .optional(),
  })
  .superRefine((value, context) => {
    if (value.action === "edit" && !value.finalVerdict)
      context.addIssue({
        code: "custom",
        message: "An edited check requires the engineer's final verdict.",
        path: ["finalVerdict"],
      });
  });
...
export async function PATCH(
  ...
  if (!parsed.success)
    return NextResponse.json(
      {
        error: "Compliance review is invalid.",
  ...
  try {
    ...
    if (existing.reviewState !== "proposed")
      return NextResponse.json(
        {
          error:
            "This compliance proposal has already received a final human disposition.",
        },
        { status: 409 },
      );
    const finalVerdict =
    ...
    if (
      ...
      existing.verdict === "needs_engineering_judgment"
    )
      return NextResponse.json(
        {
          error:
            "Needs-engineering-judgment cannot be accepted unchanged. Edit it to an explicit disposition or reject it.",
        },
        { status: 409 },
      );
    const acceptedDeviation =
      parsed.data.action !== "reject" &&
      ["deterministic_flag", "possible_mismatch"].includes(finalVerdict);

    const outcome = await db.transaction(async (tx) => {
      let finding = existing.proposedFindingId
        ? await tx.query.findings.findFirst({
            where: eq(findings.id, existing.proposedFindingId),
          })
        : undefined;
      if (acceptedDeviation && !finding) {
        const gateEdge = await tx.query.edges.findFirst({
          where: and(
            eq(edges.projectId, existing.projectId),
            eq(edges.fromType, "requirement"),
            eq(edges.fromId, existing.requirementId),
            eq(edges.relationshipType, "AFFECTS"),
            eq(edges.toType, "gate"),
          ),
        });
        const gateId = gateEdge?.toId ?? null;
        const requirement = await tx.query.requirements.findFirst({
          where: eq(requirements.id, existing.requirementId),
        });
        const severity =
          finalVerdict === "deterministic_flag" ? "high" : "medium";
        ...
        [finding] = await tx
          ...
          .values({
            projectId: existing.projectId,
            gateId,
            title: `Accepted compliance ${finalVerdict.replaceAll("_", " ")}`,
            description: `${existing.reason}\n\nEngineer disposition: ${parsed.data.note}`,
            severity,
            status: "open",
            ownerId,
            dueAt: findingDueAt(severity),
          })
          .returning();
      } else if (finding) {
        ...
        // proposal's.
        const backfillOwnerId =
          finding.ownerId ??
          (await deriveFindingOwner(
            tx,
            existing.projectId,
            finding.gateId,
            null,
        ...
        [finding] = await tx
          ...
          .set({
            ...
            title: acceptedDeviation
              ? `Accepted compliance ${finalVerdict.replaceAll("_", " ")}`
              : finding.title,
      ...
      const [check] = await tx
        ...
        .where(
          and(
            eq(complianceChecks.id, checkId),
            eq(complianceChecks.version, parsed.data.expectedVersion),
      ...
      if (parsed.data.action === "edit" || parsed.data.action === "reject") {
        await captureTeachbackNote(tx, {
          ...
          correctedTo:
            parsed.data.action === "edit" ? { verdict: finalVerdict } : null,
      ...
      await writeAuditEventInTransaction(tx, {
        ...
        action: `compliance.check.${parsed.data.action === "reject" ? "rejected" : parsed.data.action === "edit" ? "edited" : "accepted"}`,
    ...
    if (error instanceof Error && error.message === "CONFLICT")
      return NextResponse.json(
        {
          error:
            "This compliance check changed after you opened it. Reload before reviewing.",
        },
        { status: 409 },
      );
    return NextResponse.json(
      {
        error:
          error instanceof Error
            ? error.message
            : "Unable to review compliance check.",
      },
      { status: error instanceof AccessError ? error.status : 500 },
    );
  }
