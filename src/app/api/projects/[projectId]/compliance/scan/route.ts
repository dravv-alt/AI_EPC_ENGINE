...
export async function POST(
  ...
  if (!parsed.success)
    return NextResponse.json(
      { error: "Invalid scan request.", issues: parsed.error.flatten() },
      { status: 400 },
    );
  try {
      ...
      ? await db
          ...
          .where(
            and(
              ...
              inArray(requirements.id, parsed.data.requirementIds),
              eq(requirements.reviewState, "accepted"),
            ),
          )
      : await db
          .select()
    ...
    const requestedCount =
      parsed.data.requirementIds?.length ?? scopedRequirements.length;
    const skipped: Array<{ requirementId: string; reason: string }> = [];
    if (parsed.data.requirementIds?.length) {
      const foundIds = new Set(
        scopedRequirements.map((requirement) => requirement.id),
      );
      for (const requirementId of parsed.data.requirementIds) {
        if (!foundIds.has(requirementId))
          skipped.push({
            requirementId,
            reason: "Not an accepted, project-scoped requirement.",
          });
      }
    }
    ...
    for (const requirement of scopedRequirements) {
      const candidates = await discoverCandidateTargets({
        projectId,
      ...
      if (!candidates.length) {
        skipped.push({
          requirementId: requirement.id,
          reason: "No candidate target regions found.",
        });
        continue;
      }
      ...
      for (const candidate of candidates) {
        ...
        const queued = await enqueueDurableJob({
          ...
          payload: {
            projectId,
            requirementId: requirement.id,
            targetSourceRegionId: candidate.targetSourceRegionId,
        ...
        items.push({
          ...
          queuedInRedis: queued.queuedInRedis,
        });
        if (queued.duplicate) {
          jobsDuplicate += 1;
          continue;
        ...
        if (queued.queuedInRedis) {
          ...
          try {
            ...
            skipped.push({
              requirementId: requirement.id,
              reason: `Inline candidate check failed: ${error instanceof Error ? error.message : "unknown error"}`,
            });
          }
        }
      }
    ...
    return NextResponse.json(
      {
        ...
        items,
        infrastructure:
          jobsInline > 0 && jobsEnqueued === 0 ? "inline-degraded" : "queued",
      },
      { status: 200 },
    );
  } catch (error) {
