...
// Slice 8: advisory-only teach-back context for a similar past resource
// correction. Read-only — never mutates the resource.
export async function GET(request: Request, { params }: { params: Promise<{ resourceId: string }> }) {
  const { resourceId } = await params;
  const resource = await db.query.scheduleResources.findFirst({ where: eq(scheduleResources.id, resourceId) });
  if (!resource) return NextResponse.json({ error: "Resource not found." }, { status: 404 });
  try {
    await requireProjectPermission(resource.projectId, "schedule:manage");
    const advisory = await surfaceTeachbackAdvisory({ projectId: resource.projectId, subjectType: "schedule_resource", queryText: resource.name });
...
export async function POST(request: Request, { params }: { params: Promise<{ resourceId: string }> }) {
  ...
  try {
    ...
    const updated = await db.transaction(async (tx) => {
      const [updated] = await tx.update(scheduleResources).set({ name: parsed.data.name ?? resource.name, capacity: parsed.data.capacity ?? resource.capacity, unit: parsed.data.unit ?? resource.unit, validationIssues: parsed.data.action === "edit" && parsed.data.resolveValidationIssues ? [] : issues, reviewState, reviewedBy: actor.userId, reviewedAt: new Date(), reviewNote: parsed.data.note, updatedAt: new Date() }).where(eq(scheduleResources.id, resource.id)).returning();
      if (parsed.data.action === "edit" || parsed.data.action === "reject") {
        await captureTeachbackNote(tx, {
          projectId: resource.projectId,
          subjectType: "schedule_resource",
          subjectId: resource.id,
          correctedFrom: { name: resource.name, capacity: resource.capacity, unit: resource.unit },
          correctedTo: parsed.data.action === "edit" ? { name: updated!.name, capacity: updated!.capacity, unit: updated!.unit } : null,
          rationale: parsed.data.note,
          sourceRegionId: resource.sourceRegionId,
          createdBy: actor.userId,
          disposition: parsed.data.action === "edit" ? "edited" : "rejected",
          embedText: resource.name
        });
      }
      return updated;
    });
