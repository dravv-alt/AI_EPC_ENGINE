...
export default async function ShipmentsPage({ searchParams }: { searchParams: Promise<{ shipment?: string; id?: string }> }) {
  ...
  const [data, items, assetRows] = await Promise.all([getProjectShellData(projectId), db.select().from(shipments).where(eq(shipments.projectId, projectId)).orderBy(desc(shipments.createdAt)), db.select({ id: assets.id, tag: assets.tag, assetType: assets.assetType }).from(assets).where(eq(assets.projectId, projectId))]);
