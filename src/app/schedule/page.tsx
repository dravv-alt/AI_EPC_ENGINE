...
export default async function SchedulePage({ searchParams }: { searchParams: Promise<{ task?: string; risk?: string; version?: string }> }) {
  ...
  return <FeatureShell projectName={data.project} projectId={projectId} eyebrow="Delivery · deterministic planning" title="Schedule" description="Ingest cited inputs, review every task and resource, solve immutable CP-SAT versions, and trace real-world event impacts without letting AI alter dates.">
    ...
    <PredictiveRiskWorkbench projectId={projectId} hasCurrentVersion={versions.length > 0} risks={riskRows} initialRiskId={focus.risk} />
    <ScheduleWorkbench projectId={projectId} tasks={tasks} resources={resources} dependencies={dependencies} versions={versions.map((version) => ({ ...version, assignments: assignments.filter((item) => item.versionId === version.id) }))} events={events} focusTaskId={focus.task} focusVersionId={focus.version} />
  </FeatureShell>;
}
