...
import { riskSignals, scheduleAssignments, scheduleDependencies, scheduleEvents, scheduleResources, scheduleRisks, scheduleTasks, scheduleVersions } from "@/lib/db/schema";
...
export default async function SchedulePage() {
  ...
  const [data, tasks, resources, dependencies, versions, events, signalRows, riskRows] = await Promise.all([
    ...
    db.select({ signal: riskSignals, taskName: scheduleTasks.name }).from(riskSignals).innerJoin(scheduleTasks, eq(riskSignals.taskId, scheduleTasks.id)).where(eq(riskSignals.projectId, projectId)).orderBy(desc(riskSignals.observedAt)).limit(100),
  ...
  return <FeatureShell projectName={data.project} eyebrow="Deterministic planning" title="Schedule management" description="Ingest cited inputs, review every task and resource, solve immutable CP-SAT versions, and trace real-world event impacts without letting AI alter dates.">
    ...
    <PredictiveRiskWorkbench projectId={projectId} hasCurrentVersion={versions.length > 0} signals={signalRows} risks={riskRows} />
