...
import { ProjectExportControl } from "@/components/project-export-control";
...
export function FeatureShell({ projectName, projectId, eyebrow, title, description, children }: { projectName: string; projectId?: string; eyebrow: string; title: string; description: string; children: React.ReactNode }) {
  return <main className="app-shell">
    ...
    <section className="workspace">
      <header className="topbar"><MobileRouteMenu /><ProjectSearchForm /><div className="topbar-actions">{env.DEMO_MODE && <span className="demo-mode-indicator" title="Representative Mumbai DC-07 data. Public demo changes are disabled.">Demo</span>}<Link className="icon-button" href="/command-center" aria-label="Open notifications"><Bell size={19} /><span className="notification-dot" /></Link><ThemeToggle /><Link className="help-button" href={{ pathname: "/help" }} aria-label="Open help"><CircleHelp size={18} /></Link>{clerkIsConfigured && <ClerkAccountControl />}<ConnectionStatus projectId={projectId} /></div></header>
      <div className="content"><section className="page-heading"><div><p className="eyebrow">{eyebrow}</p><h1>{title}</h1><p className="subhead">{description}</p></div><div className="page-heading-actions"><ProjectExportControl projectId={projectId} title={title} /><Link className="button button-secondary" href="/"><Grid2X2 size={17} /> Project overview</Link></div></section>{children}</div>
    </section>
