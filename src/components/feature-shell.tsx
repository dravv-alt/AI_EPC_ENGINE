...
export function FeatureShell({ projectName, projectId, eyebrow, title, description, children }: { projectName: string; projectId?: string; eyebrow: string; title: string; description: string; children: React.ReactNode }) {
  return <main className="app-shell">
    ...
    <section className="workspace">
      ...
      <div className="content"><section className="page-heading"><div><p className="eyebrow">{eyebrow}</p><h1>{title}</h1><p className="subhead">{description}</p></div><Link className="button button-secondary" href="/"><Grid2X2 size={17} /> Project overview</Link></section>{children}</div>
    </section>
