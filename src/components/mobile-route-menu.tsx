...
export function MobileRouteMenu() {
  return <details className="mobile-route-menu mobile-only">
    ...
    <nav aria-label="Responsive navigation">
      {primaryWorkspaceLinks.map(([Icon, label, href]) => <Link href={href} key={href}><Icon size={17} /><span>{label}</span></Link>)}
      {workspaceGroups.map((group) => <section className="mobile-route-group" key={group.label}><h2>{group.label}</h2>{group.links.map(([Icon, label, href]) => <Link href={href} key={href}><Icon size={17} /><span>{label}</span></Link>)}</section>)}
