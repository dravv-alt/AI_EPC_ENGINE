...
export default function ShipmentMap({ shipments, selectedId }: { shipments: MapShipment[]; selectedId?: string }) {
  ...
  return <MapContainer className="shipment-map" center={center} zoom={5} scrollWheelZoom>
    ...
    {shipments.map((shipment) => {
      ...
      return <span key={shipment.id}>{routes.map((segment, index) => <Polyline positions={segment} pathOptions={{ color: color(shipment.status), weight: selected?.id === shipment.id ? 4 : 2, opacity: selected?.id === shipment.id ? .9 : .4 }} key={`${shipment.id}-${index}`} />)}{current && <CircleMarker center={current} radius={selected?.id === shipment.id ? 9 : 6} pathOptions={{ color: "#fff", weight: 2, fillColor: color(shipment.status), fillOpacity: 1 }}><Popup><b>{shipment.name}</b><br />{shipment.positionSource === "aisstream" ? "Live AIS position (aisstream.io)" : shipment.positionSource === "live" ? "Live AIS position" : "Simulated position"}<br />ETA estimate: {new Intl.DateTimeFormat("en-IN", { dateStyle: "medium", timeStyle: "short" }).format(new Date(shipment.weatherAdjustedEta ?? shipment.plannedEta))}</Popup></CircleMarker>}</span>;
    })}
  </MapContainer>;
}
