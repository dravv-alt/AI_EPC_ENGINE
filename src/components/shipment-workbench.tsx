...
export function ShipmentWorkbench({ projectId, initialShipments, assets }: { projectId: string; initialShipments: Shipment[]; assets: Asset[] }) {
  ...
  const activeShipments = shipments.filter(s => s.status?.toLowerCase() !== 'completed' && s.status?.toLowerCase() !== 'delivered');
  ...
  return <div className="workflow-stack">
    <details className="surface history-panel"><summary>Register single-leg shipment</summary>
      <form className="shipment-registration" onSubmit={submit}>
        ...
        <button className="button button-primary" disabled={saving || !assets.length}><Ship size={16} />Register shipment</button>
      </form>
    </details>
    ...
    <section className="shipment-layout">
      ...
      <aside className="surface shipment-navigator">
        <div className="section-heading">
          ...
          <button className="icon-button" aria-label="Refresh shipments" onClick={refresh}><RefreshCw size={16} /></button>
        </div>
        {activeShipments.map((shipment) => 
          <article className={`shipment-row ${selectedId === shipment.id ? "is-selected" : ""}`} key={shipment.id}>
            <button onClick={() => setSelectedId(shipment.id)}>
              <LocateFixed size={16} />
              <div>
            ...
            <form className="shipment-update" onSubmit={(event) => update(shipment, event)}>
              ...
              <button className="button button-secondary" disabled={saving}>Recalculate</button>
              <button type="button" className="button button-secondary" onClick={() => assessRoute(shipment)} disabled={saving} style={{ marginLeft: 6 }}>Assess Route</button>
              {shipment.status !== "delivered" && <button type="button" className="button button-secondary" onClick={() => markDelivered(shipment)} disabled={saving} style={{ marginLeft: 6, borderColor: "#16a34a", color: "#16a34a" }}>Complete</button>}
            </form>
            ...
            {threatAssessments[shipment.id] && (
              <div className="threat-panel" style={{ marginTop: '12px', padding: '12px', background: 'var(--surface-sunken, #f8fafc)', border: '1px solid var(--border)', borderRadius: '6px' }}>
                <h4 style={{ color: '#ef4444', marginBottom: '8px', fontSize: '0.9rem', display: 'flex', alignItems: 'center', gap: '6px' }}><AlertTriangle size={16} /> THREAT DETECTED</h4>
        ...
        {shipments.length > 0 && activeShipments.length === 0 && <div className="empty-state"><h2>No shipments to deliver</h2><p>All shipments have been marked as completed.</p></div>}
