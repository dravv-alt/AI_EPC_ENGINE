...
export function SourceUploadForm({ projectId }: { projectId: string }) {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    setState({ kind: "success", message: `Processed ${body.regionCount} citation region${body.regionCount === 1 ? "" : "s"}.` });
    router.refresh();
  }

  return <form id="source-upload" className="upload-form" ref={formRef} onSubmit={submit}>
    <label>Source title<input name="title" required minLength={3} maxLength={300} placeholder="e.g. CHW commissioning procedure" /></label>
    ...
    <input name="documentType" type="hidden" value="procedure" readOnly />
    <button className="button button-primary" disabled={state.kind === "saving"} type="submit"><FileUp size={16} /> {state.kind === "saving" ? "Processing…" : "Upload & parse"}</button>
