...
import { ArrowRight, CheckCircle2, CircleDot, FileText, Filter, GitBranch, Network, Plus, Search, ShieldAlert } from "lucide-react";
...
export function GraphWorkbench({ projectId, nodes, edges, audit }: { projectId: string; nodes: Node[]; edges: Edge[]; audit: AuditEntry[] }) {
  const router = useRouter();
  const [selectedId, setSelectedId] = useState(nodes.find((node) => edges.some((edge) => edge.fromId === node.id || edge.toId === node.id))?.id ?? nodes[0]?.id ?? "");
  ...
  const visibleEdges = edges.filter((edge) => layout.positions.has(edge.fromId) && layout.positions.has(edge.toId));
  ...
  return <div className="traceability-workbench">
    ...
    <section className="trace-studio">
      <div className="trace-canvas-panel">
        <header className="trace-toolbar"><div><p className="eyebrow">Live project provenance</p><h2>Relationship map</h2><small>{storedCount} controlled · {derivedCount} database-derived</small></div><div className="trace-controls"><label><Search size={15} /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Find an entity" /></label><label><Filter size={14} /><select value={typeFilter} onChange={(event) => setTypeFilter(event.target.value)}><option value="all">All entity types</option>{allTypes.map((type) => <option value={type} key={type}>{TYPE_LABELS[type] ?? readable(type)}</option>)}</select></label><button type="button" className={showAll ? "is-active" : ""} onClick={() => setShowAll((value) => !value)}>{showAll ? "Hide isolated" : "Show all"}</button></div></header>
        ...
        <div className="trace-canvas-scroll" tabIndex={0} aria-label="Scrollable traceability relationship graph">
          <svg className="trace-svg" viewBox={`0 0 1090 ${layout.height}`} role="img" aria-label={`${visibleNodes.length} visible entities and ${visibleEdges.length} visible relationships`}>
            ...
            <g className="trace-edges">{visibleEdges.map((edge) => { const from = layout.positions.get(edge.fromId)!; const to = layout.positions.get(edge.toId)!; const x1 = from.x + 142; const y1 = from.y + 20; const x2 = to.x; const y2 = to.y + 20; const bend = Math.max(38, Math.abs(x2 - x1) * .45); const related = selectedId === edge.fromId || selectedId === edge.toId; const dimmed = Boolean(selectedId && !related); const direction = x2 >= x1 ? 1 : -1; const path = `M ${x1} ${y1} C ${x1 + bend * direction} ${y1}, ${x2 - bend * direction} ${y2}, ${x2} ${y2}`; return <g className={`${edge.derived ? "is-derived" : "is-stored"} ${related ? "is-related" : ""} ${dimmed ? "is-dimmed" : ""}`} key={edge.id}><path d={path} markerEnd="url(#trace-arrow)" />{related && <text x={(x1 + x2) / 2} y={(y1 + y2) / 2 - 7} textAnchor="middle">{RELATION_LABELS[edge.relationshipType] ?? readable(edge.relationshipType)}</text>}</g>; })}</g>
            <g className="trace-nodes">{visibleNodes.map((node) => { const position = layout.positions.get(node.id)!; const selectedNode = selectedId === node.id; const related = neighborIds.has(node.id); const dimmed = Boolean(selectedId && !selectedNode && !related); return <g role="button" tabIndex={0} aria-label={`${TYPE_LABELS[node.type] ?? node.type}: ${node.label}`} className={`trace-node type-${node.type} ${selectedNode ? "is-selected" : ""} ${related ? "is-related" : ""} ${dimmed ? "is-dimmed" : ""}`} transform={`translate(${position.x} ${position.y})`} onClick={() => setSelectedId(node.id)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") setSelectedId(node.id); }} key={node.id}><rect width="142" height="42" rx="9" /><circle cx="13" cy="13" r="4" /><text className="trace-node-label" x="23" y="16">{short(node.label)}</text><text className="trace-node-meta" x="13" y="32">{TYPE_LABELS[node.type] ?? readable(node.type)} · {short(readable(node.state), 14)}</text></g>; })}</g>
          </svg>
        </div>
      </div>

      <aside className="trace-detail-rail">
        {selected ? <><header><div><span className={`status-pill ${stateTone(selected.state)}`}>{readable(selected.state)}</span><small>{TYPE_LABELS[selected.type] ?? readable(selected.type)}</small></div><h2>{selected.label}</h2><code>{selected.id}</code></header><section><div className="trace-section-title"><h3>Relationship story</h3><span>{selectedEdges.length}</span></div>{selectedEdges.length ? <div className="trace-story-list">{selectedEdges.map((edge) => { const outgoing = edge.fromId === selected.id; const other = nodeById.get(outgoing ? edge.toId : edge.fromId); return <button type="button" onClick={() => other && setSelectedId(other.id)} key={edge.id}><span className={`trace-relation-mark relation-${edge.relationshipType.toLowerCase()}`} /><p><b>{outgoing ? "This record" : other?.label ?? "Related record"}</b> {RELATION_LABELS[edge.relationshipType] ?? readable(edge.relationshipType)} <b>{outgoing ? other?.label ?? "related record" : "this record"}</b>.</p><small>{edge.derived ? "Database relationship" : "Controlled relationship"} <ArrowRight size={12} /></small></button>; })}</div> : <p className="trace-empty">This entity has no connected relationship yet.</p>}</section>
          <section><div className="trace-section-title"><h3><FileText size={15} /> Linked documents</h3><span>{expansion?.documents.length ?? 0}</span></div>{loading ? <p className="trace-empty">Loading context…</p> : expansion?.documents.length ? expansion.documents.map((document) => <article className="trace-document" key={`${document.documentId}-${document.evidenceId}`}><b>{document.title}</b><small>{document.documentType} · revision {document.revision}</small></article>) : <p className="trace-empty">No source document resolves through this neighbourhood.</p>}</section>
