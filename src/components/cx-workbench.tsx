...
export function CxWorkbench(props: { projectId: string; systems: System[]; gates: Gate[]; assets: Asset[]; standards: Standard[]; checklists: Checklist[]; steps: Step[]; citations: Citation[]; records: RecordRow[]; results: ResultRow[]; people: Person[] }) {
  ...
  const demoSystem = systems.find((system) => system.name.toLowerCase().includes("chilled water") && gates.some((gate) => gate.systemId === system.id) && assets.some((asset) => asset.systemId === system.id)) ?? systems.find((system) => gates.some((gate) => gate.systemId === system.id) && assets.some((asset) => asset.systemId === system.id));
