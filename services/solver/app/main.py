...
def solve(request: SolveRequest):
    horizon = max(sum(task.duration_hours for task in request.tasks) + max((task.earliest_offset for task in request.tasks), default=0), max((task.deadline_offset or 0 for task in request.tasks), default=0), 1)
    ...
    for task in request.tasks:
        ...
        if task.deadline_offset is not None:
            model.add(ends[task.id] <= task.deadline_offset)
    ...
    if status not in (cp_model.OPTIMAL, cp_model.FEASIBLE):
        return {"status": status_name, "assignments": [], "objective_hours": None, "critical_task_ids": [], "bottlenecks": ["Deadline, fixed-date, dependency, or resource constraints are mutually infeasible."]}
    ...
    return {"status": status_name, "assignments": assignments, "objective_hours": objective, "critical_task_ids": critical, "bottlenecks": []}
