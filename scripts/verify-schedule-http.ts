...
async function main() {
  ...
  try {
    ...
    assert.equal(solved.solver.status, "OPTIMAL"); assert.ok(solved.solver.objective_hours >= 18); assert.ok(solved.solver.assignments.length >= 3); assert.ok(solved.solver.critical_task_ids.includes(taskIds[2])); assert.ok(["mock", "gemini", undefined].includes(solved.explanationProvider));
    ...
    assert.equal(job.status, "completed"); assert.equal(job.result.action, "re_solved"); assert.ok(job.result.warmStartHintCount >= 3); versionIds.push(job.result.versionId);
