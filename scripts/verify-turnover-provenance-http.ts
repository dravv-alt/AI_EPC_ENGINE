...
async function main() {
  ...
  try {
    ...
    const captured = await post(`/api/projects/${projectId}/evidence`, { systemId, evidenceType: "test_reading", capturedAt: new Date().toISOString(), contentHash: "c".repeat(64) }, cookie); evidenceId = captured.evidence.id; assert.equal(captured.evidence.validityState, "pending");
