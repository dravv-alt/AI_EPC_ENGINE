...
| `schedule_resources` | Stores AI-proposed/human-reviewed resource-capacity records (crew/equipment counts) per vendor | Gemini extraction agent/reviewer | Solver assembly, review queue |
...
| `schedule_assignments` | Stores a task's computed start/end dates and critical-path flag for one schedule version | CP-SAT solver microservice call (via local job) | Schedule/critical-path view, diff/explanation |
...
| `risk_signals` | Stores a per-poll external forward-risk signal observation (or explicit data-unavailable state) backing the "Live Events" surface | Predictive Schedule Risk Engine periodic-poll worker | "Live Events" surface, risk evaluation |
...
| `auth_sessions` | Stores an opaque persisted session (hashed token, MFA-verification time, revocation, user agent/IP) for the owned-credentials auth provider | Credentials auth provider | Session validation, fresh-TOTP gate checks, revocation |
| `storage_objects` | Records an immutable stored object (key, media type, byte size, SHA-256) for every uploaded source, evidence artifact, and turnover pack | Object-storage layer | Signed reads, hash verification, provenance |
| `durable_jobs` | Stores durable job state (queue, status, idempotency key, payload, result, attempts) so BullMQ work survives restarts and is replay-safe | Job dispatch layer | Worker execution, retry/backoff, event processing audit |
| `idempotency_records` | Stores a scope+key request hash and its captured response so a retried mutation replays instead of duplicating | API idempotency middleware | Duplicate-submission protection on mutating routes |
| `turnover_packs` | Stores an approved-gate turnover manifest, its canonical hash, and the immutable object key backing independent verification | Turnover generation (approved decisions only) | Turnover surface, manifest verification, exports |
| `schedule_dependencies` | Stores a predecessor→successor precedence pair used to assemble the solver's dependency DAG | Reviewer/schedule input pipeline | Dependency validation, CP-SAT model assembly |
| `schedule_task_resources` | Stores a task's demand against a named resource — the many-to-many capacity link the solver consumes | Reviewer/schedule input pipeline | CP-SAT cumulative-capacity constraints |
| `compliance_precedents` | Stores a reviewer-accepted exact-citation precedent (requirement + target region + content hash) so an identical future comparison reuses the human's prior disposition | Compliance review (human acceptance) | Exact-hash precedent lookup on later compliance checks |
| `teachback_notes` | Stores a reviewer's correction (proposed-vs-decided snapshots, rationale, cited region, embedding) captured on edit/reject across all generic review routes | Any generic review route (edit/reject only) | Read-only advisory context on similar later reviews; never auto-applied |
...
| resource_requirement_json | TEXT | NULL | JSON-encoded resource type/quantity demand consumed against `schedule_resources` capacity |
...
- `tenants` 1:N `schedule_tasks`, `schedule_resources`, `schedule_versions`, `schedule_assignments`, and `schedule_events` (tenant predicate mirrors every other project-scoped table).
- `tenants` 1:N `cx_checklists`, `cx_test_records`, `shipments`, `compliance_checks`, `schedule_risks`, `risk_signals`, `knowledge_chunks`, and `alerts` — the new agent parent tables carry the same tenant predicate; their child tables (`cx_checklist_steps`, `cx_clause_citations`, `cx_step_results`) are scoped through the parent FK, mirroring `test_steps` under `test_procedures`.
...
- `projects` 1:N `schedule_tasks`, `schedule_resources`, `schedule_versions`, and `schedule_events` — every schedule row carries the same `(tenant_id, project_id)` predicate pair as evidence-side tables.
- `projects` 1:N `cx_checklists`, `cx_test_records`, `shipments`, `compliance_checks`, `schedule_risks`, `risk_signals`, `knowledge_chunks`, and `alerts` — same `(tenant_id, project_id)` predicate pair.
- `documents` 1:N `document_versions`; `document_versions` 1:N `source_regions`.
- `source_regions` 1:N `requirements` (existing) and 1:N `schedule_tasks`/`schedule_resources` (new) — the same citation table backs both proposal pipelines; a `source_region` row is never duplicated per module. The Commissioning agent's ingested standards/procedures follow the same discipline: `source_regions` 1:N `cx_clause_citations` (a verified citation resolves to exactly one ingested clause region). The Specification agent cites both the requirement clause and the target line as `source_regions` rows on each `compliance_checks` row, and the RFI agent's `knowledge_chunks` each point at exactly one `source_regions` citation.
...
- `schedule_tasks` 1:N `schedule_assignments` (one computed-state row per version the task appears in); `schedule_tasks` 1:N `schedule_events` (a task can accumulate multiple reported events over time). Cascade: deleting/retention-purging a `schedule_task` project cascades to its `schedule_assignments` and `schedule_events` rows (mirrors project-deletion cascade for evidence-side child tables); a `schedule_task` itself is never hard-deleted once referenced by a `schedule_assignments` row in a persisted `schedule_version` — only soft-rejected via `review_state`.
- `schedule_versions` 1:N `schedule_assignments` (the version's full task-date/critical-path snapshot). Cascade: none — `schedule_assignments` rows are immutable children of their `schedule_version` and are only ever inserted, never deleted independently of a full project purge.
...
- `schedule_resources` are associated with tasks only indirectly (via `vendor` and `resource_type` matching consumed inside `schedule_tasks.resource_requirement_json`, resolved by the solver at solve time, not by a database FK), since one resource-capacity record can constrain many tasks concurrently and the relationship is a capacity constraint, not a per-row reference.
...
All entities, including the schedule module's tables and the five agents' durable tables, are stored relationally in **local Postgres (via Drizzle ORM)** with foreign keys, `CHECK` enums, and tenant/project predicates enforced at the database layer — no separate document/graph store is introduced. `schedule_versions` and `schedule_assignments` are the one deliberate exception to normal mutability: once a version is solved, its `schedule_assignments` rows and the version's own solve-outcome fields are treated as write-once (only the later `explanation_*` fields and `schedule_events.actual_status`-driven fields on `schedule_assignments` are appended after the fact), matching the existing "no authoritative value stored as an unversioned mutable flag" rule already applied to gate readiness. Local object store (MinIO/filesystem), Postgres full-text search (tsvector), and pgvector usage extend to the new tables: Postgres full-text search additionally indexes `schedule_tasks.name`/`vendor` for citation-style lookup, and pgvector serves internal extraction-time task-to-system/asset and clause/precedent matching **and is now additionally user-facing** for the Project Knowledge & RFI Intelligence Agent's scoped semantic search over `knowledge_chunks` — always applied after the mandatory-first deterministic metadata filter and scoped to the filtered+routed subset, never a global query.
...
- Schedule dates, critical-path flags, and feasibility are derived only from a CP-SAT solver call and stored only in `schedule_versions`/`schedule_assignments`; they are never a manually editable source-of-truth column and never feed the deterministic readiness computation.
...
- The agent parent tables (`cx_checklists`, `cx_test_records`, `shipments`, `compliance_checks`, `schedule_risks`, `risk_signals`, `knowledge_chunks`, `alerts`) include both `tenant_id` and `project_id` with the same predicate rule; their child tables are scoped through the parent FK, mirroring `test_steps` under `test_procedures`.
...
- Alerts and `risk_signals` are read/cross-link and observational surfaces only — they never set gate readiness, close a finding, or alter a schedule date; the systems of record remain the underlying `schedule_events`/`findings`/`edges`/`schedule_versions` rows.
